...
Offers ways to stream values generated by properties or changes triggered by JavaFX events with
...
There is one additional type of stream, an invalidation stream, which is JavaFX specific. This
stream type does not emit values but instead just emits a `null` of type `Void`. It is distinguished
